...
class KernelClient(ConnectionFileMixin):
    ...
    * control: for kernel management calls to the kernel.
        ...
        if not isinstance(self.parent, KernelManager):
            ...
            while not await self._async_is_alive():
                if time.time() > abs_timeout:
                    raise RuntimeError(
                        "Kernel didn't respond to heartbeats in %d seconds and timed out"
                        % timeout
                    )
    ...
    async def _stdin_hook_default(self, msg: t.Dict[str, t.Any]) -> None:
        ...
        if not (
            await self.stdin_channel.msg_ready() or await self.shell_channel.msg_ready()
        ):
            self.input(raw_data)
    ...
    def shell_channel(self) -> t.Any:
        ...
        if self._shell_channel is None:
            ...
            self._shell_channel = (
                self.shell_channel_class(  # type:ignore[call-arg,abstract]
    ...
    def iopub_channel(self) -> t.Any:
        ...
        if self._iopub_channel is None:
            ...
            self._iopub_channel = (
                self.iopub_channel_class(  # type:ignore[call-arg,abstract]
                    socket, self.session, self.ioloop
                )
            )
    ...
    def stdin_channel(self) -> t.Any:
        ...
        if self._stdin_channel is None:
            ...
            self._stdin_channel = (
                self.stdin_channel_class(  # type:ignore[call-arg,abstract]
                    socket, self.session, self.ioloop
                )
            )
    ...
    def control_channel(self) -> t.Any:
        ...
        if self._control_channel is None:
            ...
            self._control_channel = (
                self.control_channel_class(  # type:ignore[call-arg,abstract]
                    socket, self.session, self.ioloop
                )
            )

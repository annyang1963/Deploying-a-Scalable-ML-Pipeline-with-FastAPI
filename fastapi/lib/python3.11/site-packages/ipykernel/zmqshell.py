...
class ZMQDisplayPublisher(DisplayPublisher):
    ...
    def clear_output(self, wait=False):
        ...
        msg = self.session.msg(
            "clear_output", json_clean(content), parent=self.parent_header
        )
...
class ZMQInteractiveShell(InteractiveShell):
    ...
    def data_pub(self):
        if not hasattr(self, "_data_pub"):
            ...
            self._data_pub.session = (
                self.display_pub.session
            )  # type:ignore[attr-defined]
            self._data_pub.pub_socket = (
                self.display_pub.pub_socket
            )  # type:ignore[attr-defined]
    ...
    def _showtraceback(self, etype, evalue, stb):
        ...
        if dh.topic:  # type:ignore[attr-defined]
            topic = dh.topic.replace(
                b"execute_result", b"error"
            )  # type:ignore[attr-defined]

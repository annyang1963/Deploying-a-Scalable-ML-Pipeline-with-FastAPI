...
class Kernel(SingletonConfigurable):
    ...
    log: logging.Logger = Instance(
        logging.Logger, allow_none=True
    ...
    async def _flush_control_queue(self):
        ...
        tracer_future: t.Union[
            concurrent.futures.Future[object], asyncio.Future[object]
        ]
    ...
    def start(self):
        ...
        control_loop = (
            self.control_thread.io_loop if self.control_thread else self.io_loop
        )

        asyncio.run_coroutine_threadsafe(
            self.poll_control_queue(), control_loop.asyncio_loop
        )
        if self.shell_stream:
    ...
    async def debug_request(self, stream, ident, parent):
        ...
        reply_content = json_clean(reply_content)
        reply_msg = self.session.send(
            stream, "debug_reply", reply_content, parent, ident
        )
        self.log.debug("%s", reply_msg)
    ...
    async def usage_request(self, stream, ident, parent):
        ...
        mem_info_type = (
            "pss" if hasattr(current_process.memory_full_info(), "pss") else "rss"
        )
        ...
        reply_msg = self.session.send(
            stream, "usage_reply", reply_content, parent, ident
        )
        self.log.debug("%s", reply_msg)

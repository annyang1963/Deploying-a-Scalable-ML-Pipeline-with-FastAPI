...
class ExtensionPackage(LoggingConfigurable):
    ...
    module = Any(
        allow_none=True,
        help="The module for this extension package. None if not enabled",
    )
    metadata = List(
        Dict(), help="Extension metadata loaded from the extension package."
    )
    version = Unicode(
        help="""
        ...
        if not linked and extension.enabled:
            try:
                ...
                if self.serverapp and self.serverapp.reraise_server_extension_failures:
                    raise
                self.log.warning(
                    "%s | error linking extension: %s", name, e, exc_info=True
                )
        ...
        if extension and extension.enabled:
            try:
                ...
                if self.serverapp and self.serverapp.reraise_server_extension_failures:
                    raise
                self.log.warning(
                    "%s | extension failed loading with message: %r",
                    name,
                    e,
                    exc_info=True,
                )

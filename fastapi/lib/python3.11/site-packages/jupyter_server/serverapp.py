        ...
        *,
        ...
        if authorizer is None:
            ...
            authorizer = AllowAllAuthorizer(
                parent=jupyter_app, identity_provider=identity_provider
            )
    ...
    def last_activity(self):
        ...
        sources.extend(
            [
                ...
                if key.endswith("_last_activity")
            ]
        )
...
class ServerApp(JupyterApp):
    ...
    token = Unicode(
        "<DEPRECATED>", help=_i18n("""DEPRECATED. Use IdentityProvider.token""")
    ).tag(config=True)
    ...
    def _deprecated_password(self, change: t.Any) -> None:
        self._warn_deprecated_config(
            change, "PasswordIdentityProvider", new_name="hashed_password"
        )
    ...
    def _default_kernel_manager_class(
        self,
    ) -> t.Union[str, t.Type[AsyncMappingKernelManager]]:
    ...
    def _default_kernel_spec_manager_class(
        self,
    ) -> t.Union[str, t.Type[KernelSpecManager]]:
        if self.gateway_config.gateway_enabled:
    ...
    no_browser_open_file = Bool(
        False,
        help="If True, do not write redirect HTML file disk, or show in messages.",
    )
    ...
    def init_configurables(self) -> None:
        ...
        if jupyter_client.version_info > (8, 3, 0):  # type:ignore[attr-defined]
            if self.allow_external_kernels:
                ...
                if external_connection_dir is None:
                    external_connection_dir = str(
                        Path(self.runtime_dir) / "external_kernels"
                    )
        ...
        if self.identity_provider_class is LegacyIdentityProvider:
            ...
            self.tornado_settings[
                "password"
            ] = self.identity_provider.hashed_password  # type:ignore[attr-defined]
    ...
    def init_event_logger(self) -> None:
        ...
        for schema_id in schema_ids:
            ...
            rel_schema_path = (
                schema_id.replace(JUPYTER_SERVER_EVENTS_URI + "/", "") + ".yaml"
            )
    ...
    def write_server_info_file(self) -> None:
        ...
        try:
            ...
            self.log.error(
                _i18n("Failed to write server-info to %s: %r"), self.info_file, e
            )

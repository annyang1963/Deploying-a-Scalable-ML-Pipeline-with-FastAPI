...
class GatewayMappingKernelManager(AsyncMappingKernelManager):
    ...
    def __init__(self, **kwargs):
        ...
        self.kernels_url = url_path_join(
            ...
            GatewayClient.instance().kernels_endpoint or "",
        )
...
class GatewayKernelSpecManager(KernelSpecManager):
    ...
    def __init__(self, **kwargs):
        ...
        self.base_endpoint = GatewayKernelSpecManager._get_endpoint_for_user_filter(
            base_endpoint
        )
    ...
    def _replace_path_kernelspec_resources(self, kernel_specs):
        ...
        for kernel_name in kernelspecs:
            ...
            for resource_name in resources:
                ...
                split_eg_base_url = str.rsplit(
                    original_path, sep="/kernelspecs/", maxsplit=1
                )
    ...
    async def get_kernel_spec_resource(self, kernel_name, path):
        ...
        self.log.debug(
            f"Request kernel spec resource '{path}' at: {kernel_spec_resource_url}"
        )
        try:
...
class GatewayKernelManager(ServerKernelManager):
    ...
    client_class = DottedObjectName(
        "jupyter_server.gateway.managers.GatewayKernelClient"
    )
    ...
    async def refresh_model(self, model=None):
        ...
        if model:  # Update activity markers
            ...
            if isinstance(self.parent, AsyncMappingKernelManager):
                ...
                self.parent._kernel_connections[self.kernel_id] = int(
                    model["connections"]
                )  # type:ignore[index]
    ...
    async def start_kernel(self, **kwargs):
        ...
        if kernel_id is None:
            ...
            if (
                os.environ.get("KERNEL_USERNAME") is None
                and GatewayClient.instance().http_user
            ):
            ...
            kernel_env = {
                ...
                or k in GatewayClient.instance().allowed_envs.split(",")
            }

            # Convey the full path to where this notebook file is located.
            if (
                kwargs.get("cwd") is not None
                and kernel_env.get("KERNEL_WORKING_DIR") is None
            ):
            ...
            self.kernel_url = url_path_join(
                self.kernels_url, url_escape(str(self.kernel_id))
            )
            self.log.info(
                f"GatewayKernelManager started kernel: {self.kernel_id}, args: {kwargs}"
            )
        else:
            ...
            self.log.info(
                f"GatewayKernelManager using existing kernel: {self.kernel_id}"
            )
    ...
    async def shutdown_kernel(self, now=False, restart=False):
        ...
        if self.has_kernel:
            ...
            try:
                ...
                self.log.debug(
                    "Shutdown kernel response: %d %s", response.code, response.reason
                )
            except web.HTTPError as error:
                if error.status_code == 404:  # noqa: PLR2004
                    self.log.debug(
    ...
    async def restart_kernel(self, **kw):
        ...
        if self.has_kernel:
            ...
            self.log.debug(
                "Restart kernel response: %d %s", response.code, response.reason
            )
    ...
    async def interrupt_kernel(self):
        ...
        if self.has_kernel:
            ...
            self.log.debug(
                "Interrupt kernel response: %d %s", response.code, response.reason
            )
...
class ChannelQueue(Queue):
    ...
    def __init__(
        self, channel_name: str, channel_socket: websocket.WebSocket, log: Logger
    ):
...
class GatewayKernelClient(AsyncKernelClient):
    ...
    * control: for kernel management calls to the kernel.
    ...
    async def start_channels(
        self, shell=True, iopub=True, stdin=True, hb=True, control=True
    ):
    ...
    def control_channel(self):
        ...
        if self._control_channel is None:
            ...
            assert self.channel_socket is not None
            self._control_channel = ChannelQueue(
                "control", self.channel_socket, self.log
            )
            assert self._channel_queues is not None

...
(JUPYTER_GATEWAY_CONNECT_TIMEOUT env var)""",
    ...
    def _connect_timeout_default(self):
        return float(
            os.environ.get(self.connect_timeout_env, self.connect_timeout_default_value)
        )
    ...
    def _request_timeout_default(self):
        return float(
            os.environ.get(self.request_timeout_env, self.request_timeout_default_value)
        )
...
(JUPYTER_GATEWAY_AUTH_HEADER_KEY env var)""",
    ...
    def _auth_header_key_default(self):
        return os.environ.get(
            self.auth_header_key_env, self.auth_header_key_default_value
        )
...
(JUPYTER_GATEWAY_AUTH_TOKEN env var)""",
    ...
    auth_scheme_default_value = (
        "token"  # This value is purely for backwards compatibility
    )
    auth_scheme = Unicode(
        allow_none=True,
        config=True,
...
environmental values to make available to the kernel. (JUPYTER_GATEWAY_ALLOWED_ENVS env var)""",
    ...
    def _allowed_envs_default(self):
        return os.environ.get(
            ...
            os.environ.get(
                "JUPYTER_GATEWAY_ENV_WHITELIST", self.allowed_envs_default_value
            ),
        )
...
(JUPYTER_GATEWAY_RETRY_MAX env var)""",
    ...
    def _gateway_retry_max_default(self):
        return int(
            os.environ.get(
                self.gateway_retry_max_env, self.gateway_retry_max_default_value
            )
        )
    ...
    def _gateway_token_renewer_class_default(self):
        return os.environ.get(
            self.gateway_token_renewer_class_env,
            self.gateway_token_renewer_class_default_value,
        )
...
(JUPYTER_GATEWAY_LAUNCH_TIMEOUT_PAD env var)""",
    ...
    def _accept_cookies_default(self):
        return bool(
            os.environ.get(
                self.accept_cookies_env, str(self.accept_cookies_value).lower()
            )
            not in ["no", "false"]
        )
    ...
    def _update_cookie_header(self, connection_args: dict) -> None:
        ...
        gateway_cookie_values = "; ".join(
            ...
            for name, (morsel, _time) in self._cookies.items()
        )
...
class RetryableHTTPClient:
    ...
    max_retries: int = int(
        os.getenv("JUPYTER_GATEWAY_MAX_REQUEST_RETRIES", MAX_RETRIES_DEFAULT)
    )

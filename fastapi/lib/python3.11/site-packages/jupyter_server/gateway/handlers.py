...
class WebSocketChannelsHandler(WebSocketHandler, JupyterHandler):
    ...
    def authenticate(self):
        ...
        if self.get_argument("session_id", None):
            ...
            self.session.session = self.get_argument(
                "session_id"
...
class GatewayWebSocketClient(LoggingConfigurable):
    ...
    def _disconnect(self):
        ...
        if self.ws is not None:
            ...
            self.log.debug(
                f"_disconnect: future cancelled, disconnected: {self.disconnected}"
            )
        ...
        while self.ws is not None:
            ...
            if not self.disconnected:
                ...
                if message is None:
                    if not self.disconnected:
                        self.log.warning(
                            f"Lost connection to Gateway: {self.kernel_id}"
                        )
                    break
                callback(
                    message
    ...
    def _write_message(self, message):
        ...
        try:
            ...
            self.log.error(
                f"Exception writing message to websocket: {e}"
            )  # , exc_info=True)
...
class GatewayResourceHandler(APIHandler):
    ...
    async def get(self, kernel_name, path, include_body=True):
        ...
        kernel_spec_res = (
            await ksm.get_kernel_spec_resource(  # type:ignore[attr-defined]
                kernel_name, path
            )
        )

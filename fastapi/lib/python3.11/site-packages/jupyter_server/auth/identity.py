...
class IdentityProvider(LoggingConfigurable):
    ...
    async def _get_user(self, handler: JupyterHandler) -> User | None:
        ...
        token_user: User | None = (
            _token_user  # need second variable name to collapse type
        )
    ...
    def get_user_cookie(
        ...
        try:
            ...
            self.log.debug(
                f"Error unpacking user from cookie: cookie={user_cookie}", exc_info=True
            )
    ...
    def process_login_form(self, handler: JupyterHandler) -> User | None:
        ...
        if self.passwd_check(typed_password) and not new_password:
            ...
            if new_password and self.allow_password_change:
                ...
                self.hashed_password = set_password(
                    new_password, config_file=config_file
                )
...
class LegacyIdentityProvider(PasswordIdentityProvider):
    ...
    def should_check_origin(self, handler: AuthenticatedHandler) -> bool:
        ...
        return self.login_handler_class.should_check_origin(
            handler
        )  # type:ignore[attr-defined]

    def is_token_authenticated(self, handler: AuthenticatedHandler) -> bool:
        """Whether we are token authenticated."""
        return self.login_handler_class.is_token_authenticated(
            handler
        )  # type:ignore[attr-defined]

...
class ZMQChannelsWebsocketConnection(BaseKernelWebsocketConnection):
    ...
    def connect(self):
        ...
        buffer_info = self.multi_kernel_manager.get_buffer(
            self.kernel_id, self.session_key
        )
    ...
    def request_kernel_info(self):
        ...
        try:
            ...
            if self.kernel_info_channel is None:
                self.kernel_info_channel = self.multi_kernel_manager.connect_shell(
                    self.kernel_id
                )
    ...
    def _limit_rate(self, channel, msg, msg_list):
        ...
        if msg_type not in {"status", "comm_open", "execute_input"}:
            ...
            elif self._iopub_msgs_exceeded and msg_rate < (
            ...
            # resume once we've got some headroom below the limit
            elif self._iopub_data_exceeded and data_rate < (

...
class FileContentsManager(FileManagerMixin, ContentsManager):
    ...
    max_copy_folder_size_mb = Int(
        500, config=True, help="The max folder size that can be copied"
    )
        ...
        try:
            ...
            if value is not None:
                ...
                try:
                    ...
                    raise TraitError(
                        "%s is outside root contents directory" % value
                    ) from None
        return ""
    ...
    def _base_model(self, path):
        ...
        if not self.allow_hidden and is_hidden(os_path, self.root_dir):
            self.log.info(
                "Refusing to serve hidden file or directory %r, via 404 Error", os_path
            )
    ...
    def rename_file(self, old_path, new_path):
        ...
        try:
            ...
            raise web.HTTPError(
                500, f"Unknown error renaming file: {old_path} {e}"
            ) from e
...
class AsyncFileContentsManager(
            ...
            for name in dir_contents:
                ...
                try:
                    if self.should_list(name) and (
                        ...
                        contents.append(
                            await self.get(path=f"{path}/{name}", content=False)
                        )
                except OSError as e:
                    # ELOOP: recursive symlink, also don't show failure due to permissions
                    if e.errno not in [errno.ELOOP, errno.EACCES]:
    ...
    async def copy(self, from_path, to_path=None):
        ...
        if model["type"] == "directory":
            ...
            # let the super class handle copying files
            return await AsyncContentsManager.copy(
                self, from_path=from_path, to_path=to_path
            )

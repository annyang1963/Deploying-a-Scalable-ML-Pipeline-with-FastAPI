...
class SessionManager(LoggingConfigurable):
    ...
    async def start_kernel_for_session(
        ...
        kernel_path = await ensure_async(
            self.contents_manager.get_kernel_path(path=path)
        )
    ...
    async def update_session(self, session_id, **kwargs):
        ...
        **kwargs : str
        ...
        if hasattr(self.kernel_manager, "update_env"):
            self.cursor.execute(
                "SELECT path, name, kernel_id FROM session WHERE session_id=?",
                [session_id],
            )
            path, name, kernel_id = self.cursor.fetchone()
            self.kernel_manager.update_env(
                kernel_id=kernel_id, env=self.get_kernel_env(path, name)
            )

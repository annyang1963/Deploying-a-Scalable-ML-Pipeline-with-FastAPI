...
class PDFExporter(LatexExporter):
    ...
    latex_count = Integer(3, help="How many times latex will be called.").tag(
        config=True
    )
    ...
    bib_command = List(
        ["bibtex", "{filename}"], help="Shell command used to run bibtex."
    ).tag(config=True)

    verbose = Bool(False, help="Whether to display the output of latex commands.").tag(
        config=True
    )

    texinputs = Unicode(help="texinputs dir. A notebook's directory is added")
    writer = Instance(
        "nbconvert.writers.FilesWriter", args=(), kw={"build_directory": "."}
    )
    ...
    def run_bib(self, filename, raise_on_failure=False):
        ...
        return self.run_command(
            self.bib_command, filename, 1, log_error, raise_on_failure
        )

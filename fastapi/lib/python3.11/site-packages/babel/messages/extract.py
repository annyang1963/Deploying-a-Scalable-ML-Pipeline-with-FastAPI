...
if TYPE_CHECKING:
    ...
    _SimpleKeyword: TypeAlias = (
        tuple[int | tuple[int, int] | tuple[int, str], ...] | None
    )
    _Keyword: TypeAlias = dict[int | None, _SimpleKeyword] | _SimpleKeyword

    # 5-tuple of (filename, lineno, messages, comments, context)
    _FileExtractionResult: TypeAlias = tuple[
        str, int, str | tuple[str, ...], list[str], str | None
    ]

    # 4-tuple of (lineno, message, comments, context)
    _ExtractionResult: TypeAlias = tuple[
        int, str | tuple[str, ...], list[str], str | None
    ]
...
def extract(
    ...
    for lineno, funcname, messages, comments in results:
        ...
        for arity in (None, len(messages)):
            ...
            if spec is None:
                spec = (1,)
            result = _match_messages_against_spec(
                lineno, messages, comments, fileobj, spec
            )
...
def extract_python(
    ...
    for tok, value, (lineno, _), _, _ in tokens:
        ...
        if current_fstring_start is not None and tok not in {
            FSTRING_START,
            FSTRING_MIDDLE,
        }:
    ...
    for token in tokenize(
        ...
        if (
            ...
            yield from parse_template_string(
                token.value, keywords, comment_tags, options, token.lineno
            )

...
def format_number(
    number: float | decimal.Decimal | str, locale: Locale | str | None = LC_NUMERIC
) -> str:
...
def format_scientific(
    ...
    pattern = parse_pattern(format)
    return pattern.apply(number, locale, decimal_quantization=decimal_quantization)
...
def parse_decimal(
    string: str, locale: Locale | str | None = LC_NUMERIC, strict: bool = False
) -> decimal.Decimal:
    ...
    if not strict and (
        group_symbol == "\xa0"
        and group_symbol  # if the grouping symbol is U+00A0 NO-BREAK SPACE,
        not in string
        ...
        if string != proper and string.rstrip("0") != (proper + decimal_symbol):
            try:
                ...
                proper_alt = format_decimal(
                    parsed_alt, locale=locale, decimal_quantization=False
                )
...
class NumberPattern:
    ...
    def _format_int(
        self, value: str, min: int, max: int, locale: Locale | str | None
    ) -> str:
        width = len(value)
        if width < min:

...
class PluralRule:
    ...
    def __repr__(self) -> str:
        rules = self.rules
        args = ", ".join(
            [f"{tag}: {rules[tag]}" for tag in _plural_tags if tag in rules]
        )
        return f"<{type(self).__name__} {args!r}>"

    @classmethod
    def parse(
        cls, rules: Mapping[str, str] | Iterable[tuple[str, str]] | PluralRule
    ) -> PluralRule:
...
def tokenize_rule(s: str) -> list[tuple[str, str]]:
    ...
    while pos < end:
        for tok, rule in _RULES:
            ...
            raise RuleError(
                "malformed CLDR pluralization rule.  " "Got unexpected %r" % s[pos]
            )
    return result[::-1]
...
def test_next_token(
    ...
    return (
        tokens and tokens[-1][0] == type_ and (value is None or tokens[-1][1] == value)
    )

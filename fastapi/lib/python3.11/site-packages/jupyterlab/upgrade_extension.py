...
def update_extension(  # noqa
    ...
    if tuple(copier.__version__.split(".")) < ("8", "0", "0"):
        copier.run_auto(
            template, output_dir, vcs_ref=vcs_ref, data=extra_context, defaults=True
        )
    else:
        copier.run_copy(
            template,
    ...
    if (
        isinstance(data.get("sideEffects"), list)
        and "style/index.js" not in data["sideEffects"]
    ):
            ...
            try:
                ...
                if config.has_section("options.extras_require"):
                    for extra, deps_raw in config.items("options.extras_require"):
                        ...
                        if extra in pyproject["project"].get(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--no-input", action="store_true", help="whether to prompt for information"
    )

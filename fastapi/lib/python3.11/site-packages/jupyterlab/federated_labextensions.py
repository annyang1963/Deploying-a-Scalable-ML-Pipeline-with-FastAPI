...
def build_labextension(
    ...
    core_path = (
        ...
        else str(Path(core_path).resolve())
    )
...
def watch_labextension(
    ...
    if ext_data["name"] not in federated_extensions:
        ...
        full_dest = pjoin(
            federated_extensions[ext_data["name"]]["ext_dir"], ext_data["name"]
        )
...
def _ensure_builder(ext_path, core_path):
    ...
    dep_version2 = dep_version2 or ext_data.get("dependencies", {}).get(
        "@jupyterlab/builder"
    )
    if dep_version2 is None:
        raise ValueError(
            "Extensions require a devDependency on @jupyterlab/builder@%s"
            % dep_version1
        )

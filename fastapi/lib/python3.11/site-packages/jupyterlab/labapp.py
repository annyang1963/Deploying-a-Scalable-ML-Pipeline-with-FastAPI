...
class LabApp(NotebookConfigShimMixin, LabServerApp):
    ...
    * App mode: JupyterLab allows multiple JupyterLab "applications" to be
    ...
    default_url = Unicode(
        "/lab", config=True, help="The default URL to redirect to from `/`"
    )
    ...
    app_dir = Unicode(
        None, config=True, help="The app directory to launch JupyterLab from."
    )
    ...
    workspaces_dir = Unicode(
        get_workspaces_dir(), config=True, help="The directory for workspaces"
    )
        ...
        if self.dev_mode:
            ...
            if not self.extensions_in_dev_mode:
                ...
                self.extra_labextensions_path = (
                    [galata_extension]
                    if galata_extension
                    in map(os.path.abspath, self.extra_labextensions_path)
                    else []
                )
        elif self.core_mode:
    ...
    def initialize_handlers(self):  # noqa
        ...
        page_config.setdefault(
            "buildAvailable", not self.core_mode and not self.dev_mode
        )
        ...
        if not self.core_mode and not errored:
            ...
            if entry_point is None:
                self.log.error(
                    f"Extension Manager: No manager defined for provider '{provider}'."
                )
            ...
            listings_config = {
                ...
                "listings_refresh_seconds": config.get(
            ...
            if len(listings_config["blocked_extensions_uris"]) or len(
                ...
                self.log.debug(
                    f"Extension manager will be constrained by {listings_config}"
                )

            try:
                ext_manager = manager_factory(
                ...
                ext_manager = ReadOnlyExtensionManager(
                    build_handler_options, listings_config, self
                )
                metadata = dataclasses.asdict(ext_manager.metadata)

    ...
    if isinstance(obj, dict):
        for key, value in obj.items():
            value = (  # noqa
                ...
                else _relax_additional_properties(value)
            )
            obj[key] = value
    ...
    if version_tuple not in validators:
        try:
            schema_json = _get_schema_json(
                v, version=version, version_minor=version_minor
            )
...
def isvalid(nbjson, ref=None, version=None, version_minor=None):
    ...
    try:
        with warnings.catch_warnings():
            ...
            validate(
                nbjson, ref, version, version_minor, repair_duplicate_cell_ids=False
            )
...
def _truncate_obj(obj):
    ...
    if isinstance(obj, dict):
        truncated_dict = {
            k: _truncate_obj(v) for k, v in list(obj.items())[:_ITEM_LIMIT]
        }
        ...
        if "cells" in error_tree:
            ...
            for cell_idx in range(number_of_cells):
                ...
                if "oneOf" in error_tree["cells"][cell_idx].errors:
                    ...
                    schemas_by_index = [
                        ref["$ref"]
                        for ref in error_tree["cells"][cell_idx]
                        .errors["oneOf"]
                        .schema["oneOf"]
                    ]
                    cell_type_definition_name = (
                        f"#/definitions/{intended_cell_type}_cell"
                    )
                    if cell_type_definition_name in schemas_by_index:
                        schema_index = schemas_by_index.index(cell_type_definition_name)
                        for error in (
                            error_tree["cells"][cell_idx].errors["oneOf"].context
                        ):
                            rel_path = error.relative_path
                            error_for_intended_schema = (
                                error.schema_path[0] == schema_index
                            )
                            ...
                            if error_for_intended_schema and is_top_level_metadata_key:
                                nbdict["cells"][cell_idx]["metadata"].pop(
                                    rel_path[1], None
                                )
                                changes += 1

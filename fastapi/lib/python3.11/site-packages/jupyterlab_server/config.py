...
def get_page_config(  # noqa: PLR0915
    ...
    for _, ext_data in federated_exts.items():
        ...
        if ext_data["jupyterlab"].get(disabled_key):
            disabled_by_extensions_all[ext_data["name"]] = ext_data["jupyterlab"][
                disabled_key
            ]
...
class LabConfig(HasTraits):
    ...
    app_namespace = Unicode("", help="The namespace of the application.").tag(
        config=True
    )

    app_url = Unicode("/lab", help="The url path for the application.").tag(config=True)

    app_settings_dir = Unicode("", help="The application settings directory.").tag(
        config=True
    )
    ...
    labextensions_path = List(
        Unicode(),
        help="The standard paths to look in for federated JupyterLab extensions",
    ).tag(config=True)

    templates_dir = Unicode("", help="The application templates directory.").tag(
        config=True
    )
    ...
    labextensions_url = Unicode(
        "", help="The url for federated JupyterLab extensions"
    ).tag(config=True)

    settings_url = Unicode(help="The url path of the settings handler.").tag(
        config=True
    )
    ...
    workspaces_api_url = Unicode(help="The url path of the workspaces API.").tag(
        config=True
    )
    ...
    translations_api_url = Unicode(
        help="The url path of the translations handler."
    ).tag(config=True)
...
def _get_config_manager(level: str) -> ConfigManager:
    ...
    return ConfigManager(
        read_config_path=[full_config_path], write_config_dir=full_config_path
    )

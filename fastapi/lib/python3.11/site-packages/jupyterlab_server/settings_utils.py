...
def _get_schema(
    ...
    if labextensions_path is not None:
        ...
        for ext_path in labextensions_path:
            target = os.path.join(
                ext_path, ext_name, "schemas", ext_name, plugin_name + ".json"
            )
...
def _get_user_settings(
    settings_dir: str, schema_name: str, schema: Any
) -> dict[str, Any]:
    """
    ...
    if os.path.exists(path):
        stat = os.stat(path)
        last_modified = tz.utcfromtimestamp(
            stat.st_mtime
        ).isoformat()  # type:ignore[no-untyped-call]

...
def timestamp(msg: dict[str, t.Any] | None = None) -> str:
    ...
    ):  # The test mocks don't provide a header, so tolerate that
...
class NotebookClient(LoggingConfigurable):
    ...
    def __init__(
        self, nb: NotebookNode, km: KernelManager | None = None, **kw: t.Any
    ) -> None:
        ...
        self.owns_km: bool = (
            km is None
        )  # whether the NotebookClient owns the kernel manager
    ...
    async def async_setup_kernel(self, **kwargs: t.Any) -> t.AsyncGenerator[None, None]:
        ...
        def on_signal() -> None:
            """Handle signals."""
            self._async_cleanup_kernel_future = asyncio.ensure_future(
    ...
    async def async_execute(
        self, reset_kc: bool = False, **kwargs: t.Any
    ) -> NotebookNode:
        """
    ...
    async def _async_handle_timeout(
        ...
        if self.interrupt_on_timeout:
            ...
            if self.error_on_timeout:
                execute_reply = {
                    "content": {**self.error_on_timeout, "status": "error"}
                }
